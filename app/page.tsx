          ...
          <div className='p-2   flex-1 '>
            ...
            <div className='bg-[#f5f5f5] h-full '>
              <form onSubmit={(e) => {
                ...
                <input className='focus-visible:outline-0 p-2 rounded-md w-full' type="text" value={value} onChange={(e) => {
                  setValue(e.target.value)
                }} />

                <button className='bg-blue-400 mx-2 rounded-md p-2 text-nowrap text-white' type='submit' > 添加 </button>
              </form>

              <div className='flex flex-col gap-2'> {list.map((n, index) => {
                return (
                  <div className='flex gap-2 items-center bg-white rounded-md overflow-hidden' key={`item_${index}`}>
                    <div className='bg-blue-400 px-4 py-2 text-white'>{index + 1}</div>
                    <div className='flex-1'>{n}</div>
                    <div className='bg-red-600 p-2 text-white' onClick={(e)=>{
